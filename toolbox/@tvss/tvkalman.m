...
elseif norm(Qn'-Qn,1) > 100*eps*norm(Qn,1)
   warning(message('Control:design:MakeSymmetric','kalman(SYS,Qn,Rn,Nn)','Qn','Qn','Qn'))
elseif norm(Rn'-Rn,1) > 100*eps*norm(Rn,1)
   warning(message('Control:design:MakeSymmetric','kalman(SYS,Qn,Rn,Nn)','Rn','Rn','Rn'))
end
...
%% Enforce Duality 
% To enforce duality and to use CRDE transpose A and C
At = A';
Ct = C';

% NOTE: CRDE function solves a problem like
%       Pdot = -A'*P - P*A - Q + EPBS*EPBS';
%
%       Riccati Diffrential Equation for LQR
%       -Pdot = A'*P + P*A - P*B*inv(R)*B'*P + G*Q*G'
%       Pdot = -A'*P - P*A + P*B*inv(R)*B'*P - G*Q*G'  {Same as CRDE}
%
%       Riccati Diffrential Equation for Kalman Filter
%       Pdot = A*P + P*A' - P*C'*inv(R)*C*P + G*Q*G'
%       Pdot = -(-A)*P - P*(-A') + P*C'*inv(-R)*C*P - (-G*Q*G') {Same as CRDE}
At = -At;
Rnbar = -Rnbar;
Qnbar = -Qnbar;
Nnbar = -Nnbar;
...
% CRDE Convention is P(T0)=F so as the TVKALMAN convention
% NOTE: F is P(t0) i.e. initial condition error covariance
Tspan = sort(Tspan(:));

%% Solve the CRDE associated with the time-varying Kalman Filter problem.
nout=nargout;
if nout<=2
    [P,L] = crde(At,Ct,Qnbar,Rnbar,Nnbar,E,F,Tspan,Opt);
else
    [P,L,Pdot,sol] = crde(At,Ct,Qnbar,Rnbar,Nnbar,E,F,Tspan,Opt);
end
