function [gFinal,wcinfo] = tvrobL2toE(G,v,p,Tf,tlmi,tSp)
%% tvrobL2toE  Worst-case L2-to-Euclidean norm
...
for i=1:Niter
    ...
    [glmi(i),X11,Plmi,Pdotlmi,Y,lamP] = ...
        FHL2toElmi(G1,v,p,Ps1,Psdot1,Pm,Pmdot);
    if DispFlag
    ...
    % [LMI1,eLMI1] = evalL2toELMI(G1,v,p,glmi(i),X11,Plmi,Pdotlmi);
    % figure; plot(eLMI1.Time,eLMI1.Data(:));
    
    % Finite Horizon: RDE + Bisection
    [gbnds,RDEinfo] = FHL2toErde(G,v,p,Tf,X11);
    grde(i) = gbnds(1);
    if isfinite(grde(i))
    ...
    if glmi(i) < grde(i)
        ...
        [~,eLMI2] = evalL2toElmi(G2,v,p,glmi(i),X11,Pval,Pdotval);
...
%% LOCAL Function: evalL2toElmi
function [LMI,eLMI] = evalL2toElmi(G,v,p,g,X11,P,Pdot)
...
DD1e = DDe(1:Nz,:);
% CC2e = CCe(Nz+1:end,:); 
% DD2e = DDe.element(Nz+1:end,:);  % Should be zero
