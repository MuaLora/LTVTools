function [g,info] = FHL2toErde(G,v,p,Tf,X11,gBnds)
...
%% Options
DispFlag = false;
OdeOpt = odeset('Events',@LOCALevents); 
OdeOpt = odeset(OdeOpt,'RelTol',1e-5,'AbsTol',1e-8);
AbsTol = 1e-4;
RelTol = 1e-3;
...
R1 = blkdiag(0,eye(Nd));
CC2eT = evalt(CC2e,Tf);
F = CC2eT'*CC2eT;  % DD2 should be zero
...
if isfinite(gUpp)
    ...
    R = R0-gTry^2*R1;
    if nout==1
        P = crde(A,B,Q,R,S,E,F,[Tf T0],OdeOpt);
        Pdot = []; sol = [];
    else
        [P,~,Pdot,sol] = crde(A,B,Q,R,S,E,F,[Tf T0],OdeOpt);
    end
